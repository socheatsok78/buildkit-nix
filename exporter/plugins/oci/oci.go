...
func (p *OCIExporterPlugin) Export(ctx context.Context, c client.Client, cfg export.ExportConfig) (export.ExportResult, error) {
	...
	if err != nil {
		return export.ExportResult{}, fmt.Errorf("nix build did not produce a manifest.json file, please check the build logs for errors: %w", err)
	}
